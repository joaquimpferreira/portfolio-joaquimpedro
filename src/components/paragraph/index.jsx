export const Paragraph = ({ children, type = 'p1' }) => {
    if (type === 'p1'){
        return <p className="font-inter text-text-700 opacity-50 text-2xl leading-7 font-normal mt-2">{children}</p>
    }
    if (type === 'p2'){
    ...
    if (type === 'p4'){
        return <p className="font-inter text-text-700 text-[16px] leading-5 font-normal">{children}</p>
    }

    if (type === 'p5'){
        return <p className="font-inter text-gray-500 text-[16px] leading-5 font-semibold">{children}</p>
    }

    
    if (type === 'p6'){
        return <p className="font-inter text-white text-[16px] leading-5 font-normal">{children}</p>
    }
